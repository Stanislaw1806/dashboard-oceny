...
# Importy do generowania PDF (ReportLab)
...
if not st.session_state["authenticated"]:
    ...
    if st.button("Zaloguj"):
        try:
            valid_users = st.secrets["credentials"]["usernames"]
            valid_passes = st.secrets["credentials"]["passwords"]
        except Exception:
            valid_users = ["stanislaw", "jacek"]
            valid_passes = ["Beza10#", "Beza10#"]
            
        login_czysty = login_input.strip().lower()
        
        if login_czysty in valid_users:
            idx = valid_users.index(login_czysty)
            if haslo_input == valid_passes[idx]:
